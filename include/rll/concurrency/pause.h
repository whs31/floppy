...
namespace rll {
   ...
   * particularly useful in low-contention scenarios, where the thread * is likely to quickly
   * acquire the lock or condition it's waiting for, * avoiding an expensive context switch. * On
   * modern x86 processors, this instruction can be invoked using @c __builtin_ia32_pause() * in
   * GCC/Clang or @c _mm_pause() in MSVC. In non-x86 architectures, alternative mechanisms such as
   * yielding the CPU may be used instead.
   *
   */
  inline void yield() {
...
#  endif
   ...
   */
  inline void yield(std::size_t count) {
    while(count-- > 0)
      yield();
  }
}  // namespace rll
